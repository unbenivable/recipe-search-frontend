...
export const useSearch = (initialIngredients = '') => {
  ...
  const [maxResultsToShow, setMaxResultsToShow] = useState(100);
  
  // Cache for search results to reduce API calls
  const [searchCache, setSearchCache] = useState({});
  const searchRequestRef = useRef(null);
  const lastSearchParamsRef = useRef('');
  const lastManualSearchRef = useRef(0);
  const autoSearchEnabledRef = useRef(false);
  ...
  const [cookingTimeFilter, setCookingTimeFilter] = useState('any');
